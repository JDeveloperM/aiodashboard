...
export default function ProposalDetailPage() {
  ...
  const handleVote = async (choice: 'for' | 'against') => {
    ...
    try {
      ...
      const result = await governanceService.submitVote(proposal.id, user.address, choice)
      
      if (result.success) {
        toast.success(`Vote submitted successfully!`, {
          description: `You voted ${choice.toUpperCase()} with voting power ${userStatus?.vote_weight || 1}`
        })
          ...
          {isSignedIn && userStatus && (
            <Card className="bg-[#1a2f51] border-[#C0E6FF]/30">
              ...
              <CardContent className="space-y-4">
                ...
                ) : userStatus.can_vote && proposal.voting_status === 'active' ? (

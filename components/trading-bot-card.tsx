...
export function TradingBotCard({
  ...
  const renderStars = () => {
    return Array.from({ length: 5 }, (_, i) => (
      <span key={i} className={cn("text-xs", i < rating ? "text-yellow-400" : "text-gray-600")}>
        ★
      </span>
    ))
  }
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center space-x-3">
              <div className="flex items-center space-x-1">
                {renderStars()}
                <span className="text-xs text-[#C0E6FF] ml-1">Rating</span>
              </div>
              <div className="flex items-center space-x-1">
                <span className="text-xs text-[#C0E6FF]">{followers?.toLocaleString() || '0'}</span>
                <span className="text-xs text-[#C0E6FF]">Subscribers</span>
              </div>
            </div>
          </div>

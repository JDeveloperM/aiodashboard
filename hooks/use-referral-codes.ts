...
import { affiliateService, type ReferralCode } from '@/lib/affiliate-service'
...
export function useReferralCodes(): ReferralCodeState & ReferralCodeActions {
  ...
  const loadReferralCodes = useCallback(async () => {
    ...
    try {
      ...
      const userCodes = await affiliateService.getUserReferralCodes(user.address)
      setCodes(userCodes)
  ...
  const createDefaultCode = useCallback(async (username: string): Promise<boolean> => {
    ...
    try {
      ...
      const success = await affiliateService.createDefaultReferralCode(user.address, username)
